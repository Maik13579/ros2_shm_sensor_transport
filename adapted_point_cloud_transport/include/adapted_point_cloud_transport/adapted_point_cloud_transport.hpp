...
#ifndef ADAPTED_POINT_CLOUD_TRANSPORT__ADAPTED_POINT_CLOUD_TRANSPORT_HPP_
#define ADAPTED_POINT_CLOUD_TRANSPORT__ADAPTED_POINT_CLOUD_TRANSPORT_HPP_
...
namespace adapted_point_cloud_transport
{
namespace detail
{
...
constexpr bool is_valid_adapter_v =
  ...
  std::is_same<RosMessageT<AdapterT>, sensor_msgs::msg::PointCloud2>::value;
...
template<typename AdapterT>
/// Type-adapted point cloud publisher with public point_cloud_transport compatibility.
class Publisher
{
  static_assert(
    detail::is_valid_adapter_v<AdapterT>,
    "adapted_point_cloud_transport::Publisher requires an rclcpp TypeAdapter whose "
    "ros_message_type is sensor_msgs::msg::PointCloud2");
...
public:
   ...
   * \param base_topic Public point-cloud topic. Transport plugins derive subtopics from it.
   * \param custom_qos QoS profile passed to public and private publishers.
   * \param public_options Options passed to the public point_cloud_transport publisher.
  ...
  : node_(std::move(node)),
    ...
    public_publisher_(point_cloud_transport::create_publisher(
        node_, base_topic_, custom_qos, public_options))
  {
...
private:
  /// Store the latest pending message and wake the executor timer.
...
public:
  ...
  /// Return only subscribers connected through point_cloud_transport topics.
...
private:
  ...
  point_cloud_transport::Publisher public_publisher_;
...
template<typename AdapterT>
/// Type-adapted point cloud subscriber with public fallback while no local publisher exists.
class Subscriber
{
  static_assert(
    detail::is_valid_adapter_v<AdapterT>,
    "adapted_point_cloud_transport::Subscriber requires an rclcpp TypeAdapter whose "
    "ros_message_type is sensor_msgs::msg::PointCloud2");
...
public:
   ...
   * falls back to the public point_cloud_transport path.
   */
  Subscriber(
    rclcpp::Node::SharedPtr node,
    std::string base_topic,
  ...
  void shutdown()
  {
    ...
    if (public_subscriber_) {
      ...
      public_subscriber_ = point_cloud_transport::Subscriber();
    }
    if (guard_condition_) {
...
private:
  /// Reconcile the active subscription with the current registry state.
  ...
  /// Subscribe to the public point_cloud_transport path and convert ROS messages to custom type.
  void create_public_subscription()
  {
    public_subscriber_ = point_cloud_transport::create_subscription(
      node_,
      base_topic_,
      [shared_callback = shared_callback_,
      unique_callback =
      unique_callback_](const sensor_msgs::msg::PointCloud2::ConstSharedPtr & message) {
        if (unique_callback) {
  ...
  point_cloud_transport::Subscriber public_subscriber_;
};

template<typename AdapterT>
/// Create a type-adapted point cloud publisher.
...
template<typename AdapterT>
/// Create a type-adapted point cloud subscriber.
...
template<typename AdapterT>
/// Create a type-adapted point cloud subscriber with owned-message callbacks.
...
#endif  // ADAPTED_POINT_CLOUD_TRANSPORT__ADAPTED_POINT_CLOUD_TRANSPORT_HPP_
