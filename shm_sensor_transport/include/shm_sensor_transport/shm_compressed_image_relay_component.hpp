...
/// @brief Composable relay from sensor_msgs::msg::CompressedImage to shared-memory metadata.
class ShmCompressedImageRelayComponent : public rclcpp::Node
{
public:
  /// @brief Construct the relay node, declare parameters, and create ROS entities.
  explicit ShmCompressedImageRelayComponent(const rclcpp::NodeOptions & options);

  /// @brief Unlink the owned shared-memory object when the relay node is destroyed.
  ~ShmCompressedImageRelayComponent() override;

private:
  /// @brief Copy one compressed image payload into shared memory and publish metadata.
  ...
  std::unique_ptr<ShmCompressedImagePublisher> shm_publisher_;
