...
: rclcpp::Node("shm_image_relay", options)
{
  ...
  metadata_publisher_ =
    create_publisher<shm_sensor_transport_interfaces::msg::ShmImage>(params_.meta_topic, qos);
...
ShmImageRelayComponent::~ShmImageRelayComponent()
{
...
bool ShmImageRelayComponent::ensure_buffer(const std::uint64_t payload_size)
{
  ...
  if (ring_.is_open() && !params_.allow_resize) {
    ...
    if (params_.warn_on_oversized_frame) {
      RCLCPP_WARN_THROTTLE(
        ...
        "Image payload is %zu bytes, larger than shared-memory slot size %zu",
