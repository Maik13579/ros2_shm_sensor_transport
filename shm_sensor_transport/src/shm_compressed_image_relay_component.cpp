...
ShmCompressedImageRelayComponent::ShmCompressedImageRelayComponent(
  const rclcpp::NodeOptions & options)
: rclcpp::Node("shm_compressed_image_relay", options)
{
  params_ = declare_relay_parameters(*this, "image_raw/compressed");
  ...
  shm_publisher_ = std::make_unique<ShmCompressedImagePublisher>(
    this, params_.input_topic, publisher_options);
  ...
  subscription_ = create_subscription<sensor_msgs::msg::CompressedImage>(
    params_.input_topic,
    qos,
    [this](sensor_msgs::msg::CompressedImage::ConstSharedPtr msg) {image_callback(msg);});
}

ShmCompressedImageRelayComponent::~ShmCompressedImageRelayComponent() = default;

void ShmCompressedImageRelayComponent::image_callback(
...
void ShmCompressedImageRelayComponent::publish_status(const std::string & message)
{
